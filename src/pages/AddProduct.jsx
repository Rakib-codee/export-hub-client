...
const AddProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to add product to server");
      ...
      // ✨ Custom Event dispatch to notify MyExports
      ...
      setToast({ type: "error", message: error.message || "Failed to add product" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center py-12 px-4 bg-linear-to-br from-slate-100 via-white to-slate-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 w-full max-w-6xl">
        <form onSubmit={handleSubmit} className="backdrop-blur-md bg-white/60 shadow-lg border border-gray-100 rounded-2xl p-8 flex flex-col gap-4">
          <h1 className="text-3xl font-bold text-center mb-4 bg-linear-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">Add Product</h1>
          ...
          <textarea className="textarea textarea-bordered w-full min-h-[100px]" name="description" placeholder="Short Description" value={form.description} onChange={handleChange} />
        ...
        <div className="rounded-2xl bg-white/60 backdrop-blur-md shadow-lg p-6 flex flex-col items-center justify-center border border-gray-100">
          {form.image ? <img src={form.image} alt={form.name} className="w-56 h-56 object-cover rounded-xl shadow-md" /> :
            <div className="w-56 h-56 bg-gray-200 rounded-xl flex items-center justify-center text-gray-400">No Image</div>}
          <div className="text-center mt-4 space-y-1">
            ...
            <p className="text-gray-800 font-medium">${form.price || "0.00"} | ⭐ {form.rating || "0.0"}</p>
            <p className="text-sm text-gray-500">Qty: {form.availableQuantity || 0}</p>
          </div>
        </div>
      </div>
    </div>
